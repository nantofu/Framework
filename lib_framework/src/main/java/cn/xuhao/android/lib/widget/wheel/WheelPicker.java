...
public class WheelPicker extends View implements IWheelPicker, Runnable {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int drawnDataPos = drawnDataStartPos + mSelectedItemPosition,
            ...
            if (isCurved) {
                ...
                // Correct unit
                int unit = 0;
                if (mDrawnItemCenterY > mDrawnCenterY) { unit = 1; } else if (mDrawnItemCenterY < mDrawnCenterY) {
                    unit = -1;
                }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                // Correct coordinates
                if (!isCyclic) {
                    if (mScroller.getFinalY() > mMaxFlingY) { mScroller.setFinalY(mMaxFlingY); } else if (
                            mScroller.getFinalY() < mMinFlingY) { mScroller.setFinalY(mMinFlingY); }
                }
                mHandler.post(this);
                if (null != mTracker) {
                    mTracker.recycle();
    ...
    private int computeDistanceToEndPoint(int remainder) {
        if (Math.abs(remainder) > mHalfItemHeight) {
            if (mScrollOffsetY < 0) { return -mItemHeight - remainder; } else { return mItemHeight - remainder; }
     ...
     * @since 2016-06-17
     */
    public abstract static class OnWheelChangeAdapter implements OnWheelChangeListener {
        /**
         * 当滚轮选择器滚动时回调该方法
         * 滚轮选择器滚动时会将当前滚动位置与滚轮初始位置之间的偏移距离返回，该偏移距离有正负之分，正值表示
         * 滚轮正在往上滚动，负值则表示滚轮正在往下滚动
